...
<div class="row">
    <div class="col-md-12">
        ...
        <!-- Account -->
        <form method="post" action="{{url($active_url).'/add'}}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-start align-items-sm-center gap-4">
                        <img src="{{asset('images/users/no-image.jpg')}}" alt="user-avatar" class="d-block rounded" height="100" width="100" id="uploadedAvatar" />
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <input class="form-control {{$errors->has('full_name')?'border-danger':''}}" type="text" name="full_name" id="full_name" value="{{session('addUser.full_name')?session('addUser.full_name'):''}}" placeholder="{{$errors->has('full_name')?$errors->first('full_name'):$translate->trans_page['full_name_holder']}}" required/>
                        </div>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="phone_number" name="phone_number" value="{{Session::has('addUser.phone_number')?Session::get('addUser.phone_number'):''}}" class="form-control" placeholder="{{$translate->trans_page['phone_number_holder']}}" />
                            </div>
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="address" class="form-label {{$errors->has('address')?'text-danger':''}}">{{$translate->trans_page['address']}}</label>
                            <input type="text" class="form-control" id="address" name="address" value="{{Session::has('addUser.address')?Session::get('addUser.address'):''}}" placeholder="{{$translate->trans_page['address_holder']}}" />
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="id_card" class="form-label {{$errors->has('id_card')?'text-danger':''}}">{{$translate->trans_page['idCard']}}</label>
                            <input type="text" class="form-control" id="id_card" name="id_card" value="{{Session::has('addUser.id_card')?Session::get('addUser.id_card'):''}}" placeholder="{{$translate->trans_page['idCard_holder']}}" />
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="email" class="form-label {{$errors->has('email')?'text-danger':''}}">{{$translate->trans_page['email']}}</label>
                            <input class="form-control" type="text" id="email" name="email" value="{{Session::has('addUser.email')?Session::get('addUser.email'):''}}" placeholder="{{$errors->has('email')?$errors->first('email'):$translate->trans_page['email_holder']}}" required/>
                        </div>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select id="default_language" name="default_language" class="select2 form-select" required>
                                ...
                                <option value="{{$lang->id}}" {{Session::has('addUser.default_language') && Session::get('addUser.default_language') == $lang->id?'selected':''}}>{{$lang->languages_name}}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="user_group" class="form-label {{$errors->has('user_group')?'text-danger':''}}">{{$translate->trans_page['group']}}</label>
                            <select id="user_group" name="user_group" class="select2 form-select" required>
                                ...
                                <option value="{{$group->id}}" {{Session::has('addUser.user_group') && Session::get('addUser.user_group') == $group->id?'selected':''}}>{{$group->name}}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 text-end">
                                    <div class="form-check form-switch text-end">
                                        <input class="form-check-input float-end" type="checkbox" role="switch" name="status" id="status" value="1" {{Session::has('addUser.status') && Session::has('addUser.status') == 1?"checked":""}}/>
                                    </div>
                                </div>
                            </div>
                        </div>
