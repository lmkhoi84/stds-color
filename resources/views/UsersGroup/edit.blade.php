...
<div class="row">
    <div class="col-md-12">
        ...
        <form method="post" action="{{url($active_url).'/edit/'.$item->id}}">
            {{ method_field('PUT') }}
            {{ csrf_field() }}
            <input type="hidden" name="group_id" id="group_id" value="{{$item->id}}">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-10">
                            <input class="form-control {{$errors->has('name')?'border-danger':''}}" type="text" name="name" id="name" value="{{Session::has('addUsersGroup.name')?Session::get('addUsersGroup.name'):$item->name}}" placeholder="{{$translate->trans_page['name_holder']}}" required/>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-10">
                            <input class="form-control {{$errors->has('sort')?'border-danger':''}}" type="text" name="sort" id="sort" value="{{Session::has('addUsersGroup.sort')?Session::get('addUsersGroup.sort'):$item->sort}}" placeholder="{{$translate->trans_page['sort_holder']}}"/>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-10">
                            <div class="form-check form-switch text-start">
                                <input class="form-check-input" type="checkbox" role="switch" name="status" id="status" value="1" {{$item->status == 1 || Session::has('addUsersGroup.status') == 1?"checked":""}}/>
                            </div>
                        </div>
                    </div>
