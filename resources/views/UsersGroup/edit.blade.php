    ...
    <div class="col-md-12">
        ...
        <form method="post" action="{{url($active_url).'/edit/'.$item->id}}" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <input class="form-control {{$errors->has('full_name')?'border-danger':''}}" type="text" name="full_name" id="full_name" value="{{session('updateUser.full_name')?session('updateUser.full_name'):$item->full_name}}" placeholder="{{$errors->has('full_name')?$errors->first('full_name'):$translate->trans_page['full_name_holder']}}" required/>
                        </div>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="phone_number" name="phone_number" value="{{Session::has('updateUser.phone_number')?Session::get('updateUser.phone_number'):$item->phone_number}}" class="form-control" placeholder="{{$translate->trans_page['phone_number_holder']}}" />
                            </div>
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="address" class="form-label {{$errors->has('address')?'text-danger':''}}">{{$translate->trans_page['address']}}</label>
                            <input type="text" class="form-control" id="address" name="address" value="{{Session::has('updateUser.address')?Session::get('updateUser.address'):$item->address}}" placeholder="{{$translate->trans_page['address_holder']}}" />
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="id_card" class="form-label {{$errors->has('id_card')?'text-danger':''}}">{{$translate->trans_page['idCard']}}</label>
                            <input type="text" class="form-control" id="id_card" name="id_card" value="{{Session::has('updateUser.id_card')?Session::get('updateUser.id_card'):$item->id_card}}" placeholder="{{$translate->trans_page['idCard_holder']}}" />
                        </div>

                        <div class="mb-3 col-md-6">
                            <label for="email" class="form-label {{$errors->has('email')?'text-danger':''}}">{{$translate->trans_page['email']}}</label>
                            <input class="form-control" type="text" id="email" name="email" value="{{Session::has('updateUser.email')?Session::get('updateUser.email'):$item->email}}" placeholder="{{$errors->has('email')?$errors->first('email'):$translate->trans_page['email_holder']}}" required/>
                        </div>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select id="default_language" name="default_language" class="select2 form-select" required>
                                ...
                                <option value="{{$lang->id}}" {{$item->default_language == $lang->id || Session::get('updateUser.default_language') == $lang->id?'selected':''}}>{{$lang->languages_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select id="user_group" name="user_group" class="select2 form-select" required>
                                ...
                                <option value="{{$group->id}}" {{$item->user_group == $group->id || Session::get('updateUser.user_group') == $group->id?'selected':''}}>{{$group->name}}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 text-end">
                                    <div class="form-check form-switch text-end">
                                        <input class="form-check-input float-end" type="checkbox" role="switch" name="status" id="status" value="1" {{$item->status == 1 || Session::has('updateUser.status') == 1?"checked":""}}/>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="text-center">
                <button type="submit" onclick="click_submit()" class="btn btn-info">{{$translate->trans_page['update']}}</button>
