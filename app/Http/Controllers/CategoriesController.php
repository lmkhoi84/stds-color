...
class CategoriesController extends Controller
{
    ...
    public function index()
    {
        ...
        $list = StructuresController::getCategoriesList($this->pid);
        // echo $this->pid;
        // echo "<pre>";
        // print_r($list);
        // echo "</pre>";
        ...
        $tree = get_tree_view($list, $this->pid, $colors, $this->main_page, $this->permissions);
        return view('categories.list', ['tree_view' => $tree, 'add' => true,'search' => true]);
    }
    // End list

    // Add new
    public function addNew($active_id = 0)
    {
        
        if ($active_id < 0){
            return redirect($this->main_page)->with(['type' => 'danger', 'alert_messenge' => $this->alert['error'] . ' ' . $this->alert['data_error']]);
        }
        if ($active_id == 0) $active_id = $this->pid;
        $this->permissions = getPermissions();
        write_file_tree_json($this->path, $this->pid, 0, $this->permissions);
        return view('categories.add', ['parent_id' => $active_id, 'langs' => $this->langs]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->permissions = getPermissions();
        write_file_tree_json($this->path, $this->pid, 0, $this->permissions, 0);
    ...
    public function edit($id)
    {
        ...
        if (Auth::user()->id != 1) {
            $permissions = array_filter(Auth::user()->menus_permission, function ($value) {
                return !is_null($value) && $value != '';
            });
        } else {
            $permissions = [];
        }
        write_file_tree_json($this->path, $this->pid, $item->id, $permissions);
        return view('categories.edit', ['item' => $item, 'langs' => $this->langs, 'name' => $name]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if (Auth::user()->id != 1){
            $permissions = array_filter(Auth::user()->menus_permission,function ($value){return !is_null($value) && $value != '';});
        }
        else $permissions = [];
    ...
    private function updateItem($data){
        ...
        $item = StructuresController::getStrucById($data['item_id']);
            $item->page_type = 2;
